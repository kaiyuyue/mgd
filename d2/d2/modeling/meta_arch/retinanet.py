...
class RetinaNet(nn.Module):
    ...
    def get_bn_before_relu(self):
        bns = self.backbone.bottom_up.get_bn_before_relu()
        bns.extend([get_norm('SyncBN', 256) for _ in range(len(self.in_features))])
        return bns

    def get_channel_num(self):
        cns = self.backbone.bottom_up.get_channel_num()
        cns.extend([256 for _ in range(len(self.in_features))])
        return cns

    def extract_feature(self, batched_inputs, preReLU=True):
        losses, fpn_outputs = self.forward(batched_inputs, return_features=True)
        images = self.preprocess_image(batched_inputs)
        feats = self.backbone.bottom_up.extract_feature(images.tensor, preReLU=preReLU)
        # aggregate bottom_up and top_down features in order of
        # [c2, c3, c4, c5, p3, p4, p5, p6, p7]
        feats.extend(fpn_outputs)
        return feats, losses
