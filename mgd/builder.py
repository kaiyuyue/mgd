...
class MGDistiller(nn.Module):
    ...
    def __init__(self,
        ...
        self.T = 1.5
        if self.with_kd:
            print('mgd info: KD will be used together, KD Temperature = {:.3f}'.format(self.T))
        ...
        self.BNs = nn.ModuleList([norm_layer(32, s) if norm_layer == nn.GroupNorm else norm_layer(s) for s in self.s_channels])
        ...
        if self.det:
            ...
            self.loss_factors = list(np.asarray([1, 1, 1e-1, 1e-1, 1, 1, 1e-2, 1e-1 * 0, 1e-1 * 0]) * 5e7)

    def init_margins(self):
        print('mgd info: init margins')
        margins = [get_margin_from_BN(bn) for bn in self.t_net.get_bn_before_relu()]
    ...
    def forward(self, x):
        ...
        loss_factors = self.loss_factors if self.det else [2 ** (feat_num - i - 1) for i in range(feat_num)]
        loss_distill = 0
        for i in range(feat_num):
            if i in self.ignore_inds:
                continue
            ...
            # accumulate loss
            loss_distill += distillation_loss(s_feats[i], t.detach(), m) / loss_factors[i]
...
class SMDistiller(nn.Module):
    ...
    def update_flow(self):
        ...
        for i in range(feat_num):
            ...
            solve_status = assignment.Solve()
            if solve_status == assignment.OPTIMAL:
                _col_ind = [assignment.RightMate(n) for n in range(0, assignment.NumNodes())]
                cost_sum = 0
                for n in range(0, assignment.NumNodes()):
                    if n <= sc:
